...
namespace MopBotTwo.Common.Systems.Dev
{
	...
	public partial class DevSystem : BotSystem
	{
		...
		[Command("notice")] public async Task NoticeMessageCommand(ulong messageId) => await NoticeMessageCommand(Context.socketTextChannel,messageId);
		[Command("notice")] public async Task NoticeMessageCommand(SocketTextChannel channel,ulong messageId)
		{
		...
		[Command("sudo")] public async Task SudoCommand(SocketGuildUser user,[Remainder]string command)
			=> await SudoCommand(user,Context.socketTextChannel,command);
		[Command("sudo")] public async Task SudoCommand(SocketGuildUser user,SocketTextChannel channel,[Remainder]string command)
			=> await CommandSystem.ExecuteCommand(new MessageExt(Context.message,Context.server,user,command,true,channel),true);

		[Command("errortest")] public async Task ErrorTest() => throw new Exception("An error has occured, as you requested!");

		[Command("listservers")] public async Task ListServers() => await Context.ReplyAsync($"Currently running for the following servers: ```\n{string.Join('\n',MopBot.client.Guilds.Select(g => $"{g.Name} - {g.Id}"))}```");
		[Command("listchannels")] public async Task ListChannels() => await Context.ReplyAsync($"The following channels exist on this server: ```\n{string.Join('\n',Context.server.Channels.Select(c => $"{c.Name} - {c.Id}"))}```");
		[Command("listroles")] public async Task ListRoles() => await Context.ReplyAsync($"The following roles exist on this server: ```\n{string.Join('\n',Context.server.Roles.Select(r => $"{r.Name} - {r.Id}"))}```");
