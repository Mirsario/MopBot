...
namespace MopBotTwo.Common.Systems.XP
{
	public partial class XPSystem
	{
		...
		public static async Task GiveXP(ulong numXP,SocketGuildUser user,SocketGuild server,SocketGuildChannel channel)
			=> await ModifyXP(xp => xp+numXP,user,server);

		[Command("take")]
		[RequirePermission(SpecialPermission.Owner)]
		public static async Task TakeXP(ulong numXP,SocketGuildUser user,SocketGuild server,SocketGuildChannel channel)
			=> await ModifyXP(xp => xp-numXP,user,server);

		[Command("set")]
		[RequirePermission(SpecialPermission.Owner)]
		public static async Task SetXP(ulong newXP,SocketGuildUser user,SocketGuild server)
			=> await ModifyXP(xp => newXP,user,server);
