...
{
	//This is a copy of the default UserTypeReader, but all user lookups have the AllowDownload flag instead of CacheOnly.
	public class DownloadCapableUserTypeReader<T> : TypeReader where T : class, IUser
	{
		public override async Task<TypeReaderResult> ReadAsync(ICommandContext context, string input, IServiceProvider services)
		{
			Console.WriteLine($"DownloadCapableUserTypeReader: ReadAsync()");
			...
			if(index >= 0) {
				...
				if(ushort.TryParse(input.Substring(index + 1), out ushort discriminator)) {
					...
					var guildUser = guildUsers
						.FirstOrDefault(x => x.DiscriminatorValue == discriminator && string.Equals(username, x.Username, StringComparison.OrdinalIgnoreCase));
