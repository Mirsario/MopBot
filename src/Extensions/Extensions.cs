...
namespace MopBotTwo.Extensions
{
	...
	public static class UnsortedExtensions
	{
		...
		public static async Task TryAddReaction(this MessageExt message,IEmote emote)
		{
			...
			await message.socketMessage.AddReactionAsync(server,emote);
		}
		...
		{
			var serverData = MemorySystem.memory[user.Guild].GetData<PermissionSystem,PermissionSystem.PermissionServerData>();
			/*var userData=	MemorySystem.memory[user.Guild][user].GetData<PermissionSystem,PermissionUserData>();
			if(permissions.Any(p => userData.permissions.Contains(p))) {
				return true;
			}*/
