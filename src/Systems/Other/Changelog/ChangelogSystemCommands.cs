...
namespace MopBotTwo.Systems
{
	public partial class ChangelogSystem : BotSystem
	{
		...
		public async Task AddFixedIssues(string asEntryType)
		{
			...
			var issueData = Context.server.GetMemory().GetData<IssueSystem,IssueSystem.IssueServerData>();
			if(!data.GetChangelogChannel(out var channel)) {
				throw new BotError("Changelog channel has not been set!");
			}

			foreach(var issue in issueData.issues) {
				if(issue.status==IssueSystem.IssueStatus.Closed) {
		...
		[RequirePermission(SpecialPermission.Owner,"changelogsystem.entries.show")]
		...
		[Command("parse")]
		[RequirePermission(SpecialPermission.Owner,"changelogsystem.entries.parse")]
