...
namespace MopBotTwo
{
	public class MessageExt : ICommandContext
	{
		...
		public MessageExt(IMessage message,SocketGuild server,SocketGuildUser user,string content = null,bool? isCommand = null,SocketTextChannel channel = null)
		{
			...
			Setup();
			this.isCommand = isCommand ?? this.content.StartsWith(server?.GetMemory()?.GetData<CommandSystem,CommandSystem.CommandServerData>()?.commandPrefix ?? MopBot.DefaultCommandPrefix);
			//Unfinished
		}
		...
		private void Setup(SocketMessage message)
		{
			...
			isCommand = content.StartsWith(server?.GetMemory()?.GetData<CommandSystem,CommandSystem.CommandServerData>()?.commandPrefix ?? MopBot.DefaultCommandPrefix);
		}
